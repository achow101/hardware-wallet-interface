...
class TestDisplayAddress(DeviceTestCase):
    ...
    def test_display_address_bad_args(self):
        result = self.do_command(self.dev_args + ['displayaddress', '--sh_wpkh', '--wpkh', 'm/49h/1h/0h/0/0'])
        self.assertIn('error', result)
        self.assertIn('code', result)
        self.assertEqual(result['code'], -7)

    def test_display_address(self):
        self.do_command(self.dev_args + ['displayaddress', 'm/44h/1h/0h/0/0'])
        self.do_command(self.dev_args + ['displayaddress', '--sh_wpkh', 'm/49h/1h/0h/0/0'])
        self.do_command(self.dev_args + ['displayaddress', '--wpkh', 'm/84h/1h/0h/0/0'])

    def test_bad_path(self):
        result = self.do_command(self.dev_args + ['displayaddress', 'f'])
